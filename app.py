...
# --- Tabs ---
tab_dashboard, tab_validation = st.tabs(["Live Dashboard", "System Benchmarks"])
...
with tab_validation:
    ...
    st.markdown("Real-time performance metrics monitored during edge deployment tests.")
    ...
    with col_v1:
        st.subheader("1. Accuracy Comparison (CNN vs SVM)")
        if acc:
    ...
    with col_v2:
        st.subheader("2. Edge Inference Stats (TFLite)")
        if perf:
    ...
    if perf and perf['latency_target_met'] and perf['size_target_met']:
        st.success("✅ System successfully optimized for Edge Deployment (meets latency/size constraints).")
    else:
        st.warning("⚠️ Optimization required for edge deployment.")
